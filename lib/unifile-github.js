...
class GitHubConnector {
  ...
  setBasicAuth(session, username, password) {
    // Create a copy to only set the true basic auth when we know it's the good one
    const sessionCopy = Object.assign({}, session);
    sessionCopy.basic = new Buffer(username + ':' + password, 'ascii').toString('base64');
    return this[callAPI](sessionCopy, '/user', null, 'GET')
    .then(this[assignSessionAccount].bind(undefined, session))
    .then(() => {
      session.basic = sessionCopy.basic;
      return username;
    });
  }
    ...
    switch (splitPath.length) {
      ...
      case 1: // List all branches
        if(!session.account || !session.account.login)
          resultPromise = Promise.reject('User account login is not set. Did you called login()?');
        else {
